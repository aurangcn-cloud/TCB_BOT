...
WAIT_AFTER_MATCH_SECONDS = 5 
START_SPAM_DELAY = 0.01
level_up_running = False
level_up_task = None
level_up_wait_time = 15
CONTROL_FILE = "bot_control.json"

def check_web_control():
    """ওয়েবসাইট থেকে command চেক করে"""
    ...
    try:
        ...
        if cmd == 'stop' and level_up_running:
            level_up_running = False
            print("🌐 Web Panel: Level Up Bot বন্ধ করা হয়েছে!")
            # clear command
            state['command'] = None
...
def start_api_servers():
    ...
    if processes:
        print(f"⏳ Waiting 1 seconds for API servers to initialize...")
        time.sleep(1)
    return processes
...
🤖 Bot Name: Shamim Ahmed
✅ Status: Ready to join

📱 Player will see:
"Shamim Ahmed wants to join your team"
...
async def black666(client_id, key, iv):
    banner_text = "[FF0000][B][C] ERROR , WELCOME TO [FFFFFF]Shamim Ahmed [00FF00]___ Shamim Ahmed____ BOT ! \n[FFFF00]NEW VERSION NEW FUNCTION !\n[FF0000]TELEGRAM : @Shamim_Ahmed\n\n"     
    fields = {
        1: 5,
        2: {
            1: int(client_id),
...
def send_tiktok_info(username):
    ...
    try:
        ...
        if user.get("credit") != "Shamim Ahmed":
...
[C][B][00FFFF]━━━━━━━━━━━
[C][B][FFFFFF]Developer     : Shamim Ahmed
"""
    # Send main info
...
[C][B][11EAFD]‎━━━━━━━━━━━━
[C][B][FFB300]Subscribe: [FFFFFF]Shamim Ahmed [00FF00]!!
"""
...
async def level_up_loop(team_code, target_uid, key, iv, region, chat_type, chat_id):
    ...
    while level_up_running and cycle_count < max_cycles:
        try:
            ...
            # ওয়েব প্যানেল থেকে stop command চেক
            check_web_control()
            ...
            # Step 5: Next cycle delay — মাত্র 1 সেকেন্ড
            await asyncio.sleep(1)
...
[00FF00]Shamim Ahmed TCP
WELCOME TO Shamim Ahmed TCP BOT
...
"""
            ...
            # For example:
            title_packet = await convert_kyro_to_your_system(uid, chat_id, key, iv, nickname="Shamim Ahmed", title_id=title_id)
...
async def noob(target_uid, chat_id, key, iv, nickname="Shamim Ahmed", title_id=None):
    """EXACT conversion with customizable title ID"""
    try:
        # Use provided title_id or get random one
        if title_id is None:
...
async def send_all_titles_sequentiallly(uid, chat_id, key, iv, region, chat_type):
    ...
    # Send initial message
    start_msg = f"""[B][C][00FF00] Noobde Shamim Ahmed GAMER ya meku agar tu noob bolra toh tu g a y hai
...
"""
    ...
    try:
        for index, title_id in enumerate(all_titles):
            ...
            # You'll need to use your existing title sending logic here
            # For example:
            title_packet = await noob(uid, chat_id, key, iv, nickname="Shamim Ahmed", title_id=title_id)
              ...
              # =================== EMOTE DETECTION ONLY ===================
                ...
                if data_hex.startswith('0500') and insquad is not None and joining_team == False:
                    try:
                        ...
                        if packet_json.get('1') in [6, 7]: 
                             insquad = None
                             joining_team = False
                             print("Squad cancelled! Spamming start continuously...")
                             try:
                                 start_packet = await start_auto_packet(key, iv, region)
                                 while insquad is None:
                                     await SEndPacKeT(whisper_writer, online_writer, 'OnLine', start_packet)
                                     await asyncio.sleep(0.05)
                             except Exception as spam_e:
                                 print(f"Start spam error: {spam_e}")
                             continue
                ...
                if data_hex.startswith("0500") and insquad is None and joining_team == False:
                    try:
                        ...
                        pkt5_raw = packet_json.get('5', {})
                        if isinstance(pkt5_raw, str):
                            continue
                        pkt5 = pkt5_raw.get('data', {}) if isinstance(pkt5_raw, dict) else {}
                        if not isinstance(pkt5, dict):
                            continue

                        uid = pkt5.get('1', {}).get('data') if isinstance(pkt5.get('1', {}), dict) else None
                        invite_uid = None
                        try:
                            invite_uid = pkt5['2']['data']['1']['data']
                        except:
                            pass
                        squad_owner = uid
                        code = pkt5.get('8', {}).get('data') if isinstance(pkt5.get('8', {}), dict) else None
                        if not uid or not code:
                            continue
                        # invite_uid None হলে uid দিয়ে চালাবে
                        if not invite_uid:
                            invite_uid = uid
                        ...
                        if "MĢ24_GÀMER" in WHITELISTED_UIDS:
                            ...
                            # ম্যাচের ভিতরে থাকতেই continuously start দিতে থাকবে
                            async def continuous_start_spam():
                                try:
                                    start_packet = await start_auto_packet(key, iv, region)
                                    while insquad is not None:
                                        await SEndPacKeT(whisper_writer, online_writer, 'OnLine', start_packet)
                                        await asyncio.sleep(0.01)
                                except Exception as e:
                                    print(f"Continuous start spam error: {e}")
                            asyncio.create_task(continuous_start_spam())

                            async def rejoin_after_match():
                                try:
                                    print("⏳ ১ সেকেন্ড পর আবার squad এ join করবে...")
                                    await asyncio.sleep(0.01)
                                    print("🔄 আবার join করছে...")
                                    global insquad, joining_team
                                    insquad = None
                                    joining_team = False
                                    Join2 = await ArohiAccepted(squad_owner, code, key, iv)
                                    await SEndPacKeT(whisper_writer, online_writer, 'OnLine', Join2)
                                    print(f"✅ Bot rejoined squad of {squad_owner}!")
                                    insquad = True
                                    start_packet2 = await start_auto_packet(key, iv, region)
                                    while insquad is not None:
                                        await SEndPacKeT(whisper_writer, online_writer, 'OnLine', start_packet2)
                                        await asyncio.sleep(0.01)
                                except Exception as e:
                                    print(f"Rejoin error: {e}")
                            asyncio.create_task(rejoin_after_match())
                            ...
                            try:
                                ...
                                bot_uid = 13777711848
                                message_text = f" Can't accept Your request Talk to Shamim Ahmed"
                ...
                if data_hex.startswith('0500') and len(data_hex) > 1000:
                    try:
                        ...
                        if packet_type in [6, 7, 8, 9, 10, 11, 12]:
                            ...
                            print(f"✅ Bot reset. Spamming start continuously...")
                            try:
                                start_packet = await start_auto_packet(key, iv, region)
                                while insquad is None:
                                    await SEndPacKeT(whisper_writer, online_writer, 'OnLine', start_packet)
                ...
                if insquad == True:
                    try:
                        ...
                        if not OwNer_UiD or not CHaT_CoDe:
                            ...
                            message = """[B][C][FF0000]█▓▒░░ WELCOME TO Shamim Ahmed BOT ░░▒▓█
[00FF00]»»————-　★　————-««
[FFFFFF]⚡ BOT POWERED BY:[FF0000] Shamim Ahmed
[FFFFFF]🎯 STATUS: [00FF00]ACTIVE 24/7
[FFFFFF]📱 CONTACT: [FFFF00]@Shamim_Ahmed
[00FF00]»»————-　★　————-««
[FF0000]█▓▒░░ ENJOY THE BOT! ░░▒▓█"""
                        ...
                        pass  # squad data না থাকলে চুপ থাকবে
...
📝 Examples:
/bio Hello World!
/bio 🤖 Bot by Shamim Ahmed
/bio Level 70 | Pro Player
/bio Add me: Shamim Ahmed
                        ...
                        if inPuTMsG.strip().startswith('/all_event'):
                            ...
                            if len(parts) < 2:
                                ...
                                for event_msg in event_results:
                                    ...
                                    await asyncio.sleep(0.05)  # 0.2 সেকেন্ড pause
...
# GUILD INFORMATION FF
                        ...
                        if inPuTMsG.strip().startswith('/lw'):
                            ...
                            if len(parts) < 2:
                                await safe_send_message(response.Data.chat_type,
                                    "[B][C][FF0000]❌ Usage: /lw <TeamCode>\nExample: /lw ABC123\n",
                                    uid, chat_id, key, iv)
                            else:
                                team_code = parts[1]
                                if level_up_running:
                                    await safe_send_message(response.Data.chat_type,
                                        "[B][C][FFFF00]⚠️ Level Up Bot ইতিমধ্যে চলছে!\n/stop দিয়ে বন্ধ করুন।\n",
                                        uid, chat_id, key, iv)
                                else:
                                    level_up_running = True
                                    level_up_task = asyncio.create_task(
                                        level_up_loop(team_code, uid, key, iv, region,
                                            response.Data.chat_type, chat_id)
                                    )
                                    await safe_send_message(response.Data.chat_type,
                                        f"[B][C][00FF00]✅ Level Up Bot চালু হয়েছে!\n🎮 Team Code: {team_code}\n🔄 Auto cycling...\n",
                                        uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip() == '/stop':
                            if level_up_running:
                                level_up_running = False
                                await safe_send_message(response.Data.chat_type,
                                    "[B][C][FF0000]🛑 Level Up Bot বন্ধ করা হয়েছে!\n",
                                    uid, chat_id, key, iv)
                            else:
                                await safe_send_message(response.Data.chat_type,
                                    "[B][C][FFFF00]⚠️ Level Up Bot এখন চলছে না।\n",
                                    uid, chat_id, key, iv)
...
[b][i][FFC0CB]youtube: Shamim Ahmed[/b]

[b][c][FFC0CB]subcribe: my_channel[FFFFFF]
 
[b][i][FFA500]telegram: @Shamim_Ahmed[/b]

[b][c][FFA500]telegram contact: @Shamim_Ahmed[A52A2A]
 
Enjoy the bot my friend.......

[C][B][0000FF] Created by Black666FF
Modified by - Shamim Ahmed
"""
...
║ ────────────── 
║                                       
║ ⚡ [FF00FF] Shamim Ahmed GAMER VIP BOT ⚡   
╚═════════════╝"""
                            await safe_send_message(response.Data.chat_type, admin_message, uid, chat_id, key, iv)
...
# In TcPChaT function, update /status command:
                        ...
                        if inPuTMsG.strip().lower() in ("help", "/help", "menu", "/menu", "hi"):
                            ...
                            header = f"[c][00FF00]Hey {player_name} [00FFFF]Welcome To Shamim Ahmed's BOT"
...
[00FFFA]║
[FFFF00]║ 👤 Developer    :: [FF1493]Shamim Ahmed
...
async def MaiiiinE():
    ...
    os.system('clear')
    print("=" * 50)
    print("🤖 Shamim Ahmed BOT - INITIALIZING")
    print("=" * 50)
    ...
    os.system('clear')
    print("🤖 Shamim Ahmed BOT - STARTING")
    print("=" * 50)
    ...
    os.system('clear')
    print("🤖 Shamim Ahmed BOT - CONNECTING")
    print("=" * 50)
    print("┌────────────────────────────────────┐")
    print("│ ██████████████████████████████████ │")
    print("└────────────────────────────────────┘")
